...
<script setup>
import { Bot, Compass, Target, ClipboardCheck, LayoutDashboard, MessageSquare, Route, FileText, Settings, ShieldCheck, Plug, Puzzle, GitPullRequest, Wrench, Search, BookOpen, Terminal, Code, Brain, Lock, Cloud, ListChecks, Boxes, Rocket, Gauge, ScrollText, LifeBuoy, Network, GitBranch } from 'lucide-vue-next'
</script>

<section class="frontpage">
  <p class="frontpage-kicker">Codex Playbook</p>
  <h1 class="frontpage-title">把 Codex 当成长期协作系统，而不是一次性聊天窗口</h1>
  <p class="frontpage-subtitle">这是一份面向独立开发者和小团队的 Codex 中文 Playbook：先讲清楚 Codex 的工作边界，再把提示词、上下文、权限、工具、扩展、Review、CI 和发布串成一条可复用的实战链路。</p>
  <div class="frontpage-rule">
    <span><strong>Goal</strong><br>目标要能判断完成</span>
    <span><strong>Context</strong><br>真相源要先给够</span>
    <span><strong>Constraints</strong><br>边界和禁区要明确</span>
    <span><strong>Done when</strong><br>验收命令要跑完</span>
  </div>
</section>

<div class="quick-grid">
  <a href="#codex-地图"><div class="card-icon"><Compass /></div><div class="card-body"><strong>Codex 地图</strong><span>CLI、IDE、App、Cloud、Review、Sites 分别适合什么任务。</span></div></a>
  <a href="#提示词契约"><div class="card-icon"><MessageSquare /></div><div class="card-body"><strong>提示词契约</strong><span>把模糊需求变成 Codex 可执行、可验证的任务描述。</span></div></a>
  <a href="#计划-目标与执行"><div class="card-icon"><Target /></div><div class="card-body"><strong>计划与目标</strong><span>什么时候先 Plan，什么时候开 Goal，什么时候拆小任务。</span></div></a>
  <a href="#上下文工程"><div class="card-icon"><FileText /></div><div class="card-body"><strong>上下文工程</strong><span>AGENTS、README、日志、截图、官方文档和真相源怎么组织。</span></div></a>
  <a href="#配置-权限与安全"><div class="card-icon"><Lock /></div><div class="card-body"><strong>权限与安全</strong><span>sandbox、approval、secrets、网络和云端环境的边界。</span></div></a>
  <a href="#扩展系统"><div class="card-icon"><Plug /></div><div class="card-body"><strong>扩展系统</strong><span>Skills、MCP、插件、Hooks、Subagents、Automations 怎么分层。</span></div></a>
  <a href="#实战工作流"><div class="card-icon"><Route /></div><div class="card-body"><strong>实战工作流</strong><span>新项目、修 bug、做 UI、改数据、上线发布的标准打法。</span></div></a>
  <a href="#review-ci-与发布"><div class="card-icon"><GitPullRequest /></div><div class="card-body"><strong>Review 与发布</strong><span>让 Codex 写完、测完、审完、交付，而不是只生成代码。</span></div></a>
  <a href="#排障手册"><div class="card-icon"><LifeBuoy /></div><div class="card-body"><strong>排障手册</strong><span>跑偏、工具缺失、上下文污染、权限卡住时怎么救回来。</span></div></a>
</div>

## 1. Codex 到底是什么 {#codex-地图}

Codex 是一个会读代码、改文件、运行命令、使用工具、做 Review、调用连接器和持续推进任务的编码代理。它不是只回答问题的聊天框，也不是只会补全代码的编辑器插件。你真正要学会的是：如何把它放进你的工程系统，让它知道什么是真相源、什么是边界、什么叫完成。

### <Compass class="cat-icon" /> 核心心智模型 {#核心心智模型}

Codex 的价值来自一条闭环：理解目标，读取上下文，制定步骤，修改文件，运行验证，解释结果，再根据反馈继续迭代。你每次使用它，都应该尽量把任务放进这条闭环，而不是只问“帮我写一个函数”。
...
CLI 的关键不是命令本身，而是启动位置。进入正确仓库、确认分支、确认当前工作树状态，再把目标讲清楚。大型任务不要直接让它“重构整个系统”，先让它读关键文件和给计划。
...
这份站点没有使用 Sites，而是复用了参考项目的 Cloudflare Workers Static Assets 路线。原因很简单：用户明确要求部署到 Cloudflare Worker，并使用自己的域名。选型不是看工具新不新，而是看交付目标在哪里。
...
很多失败不是模型能力问题，而是你让它在没有真相源的情况下开始写。真正的工作流应该是：先让它定位权威文档、当前代码、运行态、日志、schema、API 合同，再实现。
...
对于更长的工程任务，可以让 Codex 维护一个 `PLANS.md` 或 `docs/plans/xxx.md`。计划文档不是给人看的仪式感，而是让任务跨上下文压缩后仍能继续推进。
...
不要为了省一次确认，把生产操作变成默认允许。真正省时间的是把可验证的安全命令沉淀成规则，而不是放开所有权限。
...
本地线程能访问你的本机文件、依赖缓存、登录状态、私有配置。云端线程通常只能访问 GitHub 仓库和配置好的 secrets。很多“本地能跑，云端失败”的问题，本质是环境不一致。
...
不要让最终总结变成流水账。用户真正需要的是“现在能不能用、怎么确认、哪里要小心”。
...
<div class="callout-strip">
  ...
  <span>把 Codex 当成一个可配置、可验证、可复盘的工程系统：先给真相源，再给边界，再让它执行，最后用测试、Review 和线上 smoke test 收口。</span>
</div>
